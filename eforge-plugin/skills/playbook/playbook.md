...
Playbook management and run behavior is owned by the `eforge-playbooks` extension. The `mcp__eforge__eforge_playbook` compatibility tool is safe to use for normal playbook workflows, but it delegates to `eforge-playbooks:*` generic extension contributions rather than direct daemon playbook routes.

Use the compatibility tool first when it is available, for example:

- `mcp__eforge__eforge_playbook { action: "list" }`
- `mcp__eforge__eforge_playbook { action: "run", name: "docs-sync" }`
- `mcp__eforge__eforge_playbook { action: "copy", name: "docs-sync", targetScope: "project-local" }`
...
- `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:list-playbooks", input: {} }`
- `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:run-playbook", input: { name: "docs-sync" } }`
- `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:copy-playbook", input: { name: "docs-sync", targetScope: "project-local" } }`
...
Call `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:list-playbooks", input: {} }` to fetch the current playbook inventory. Use the result to determine which branches to show:
...
1. Call `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:validate-playbook", input: { raw: "<draft-markdown>" } }` before saving.
...
2. Call `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:save-playbook", input: { scope: "<scope>", playbook: { frontmatter: {...}, body: {...} } } }`.
...
Call `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:list-playbooks", input: {} }`. Print a numbered list:
...
If the user picks option 3, call `mcp__eforge__eforge_playbook { action: "copy", name: "<name>", targetScope: "project-local" }` before entering the section-by-section edit loop. This atomically copies the playbook to the project-local tier so future invocations resolve to the new shadow. Then proceed with the edit loop using the copied version.

### 4.3: Load the playbook

Call `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:show-playbook", input: { name: "<name>" } }` (resolved to the tier the user chose).
...
Call `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:show-playbook", input: { name: "<name>" } }` to get the full playbook content including mode, Goal, Acceptance criteria, and Notes for the planner.
...
- **Run now**: Call `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:run-playbook", input: { name: "<name>" } }`.
- **Wait for build**: Call `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:run-playbook", input: { name: "<name>", afterQueueId: "<resolved-id>" } }`.

For **explicit selections** (`landingActionOverride` is set), include `landingAction`:
- **Run now**: Call `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:run-playbook", input: { name: "<name>", landingAction: "<landingActionOverride>" } }`.
- **Wait for build**: Call `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:run-playbook", input: { name: "<name>", afterQueueId: "<resolved-id>", landingAction: "<landingActionOverride>" } }`.
...
1. **Check the capability through the playbook run response**: Call `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:run-playbook", input: { name: "<name>" } }`. This does not enqueue planning-mode playbooks. It checks the required `eforge.plan.planning-mode-playbook` capability.
...
Call `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:list-playbooks", input: {} }` and render a formatted read-only listing.
...
Call `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:list-playbooks", input: {} }` and filter for `source: "project-local"` entries. Present as a numbered list. If none exist, tell the user.
...
Call `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:promote-playbook", input: { name: "<name>" } }`.
...
Call `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:list-playbooks", input: {} }` and filter for `source: "project-team"` entries. Present as a numbered list. If none exist, tell the user.
...
Call `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:demote-playbook", input: { name: "<name>" } }`.
...
Every save path (Create and Edit) must pass `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:validate-playbook", input: { raw: "<markdown>" } }` before the extension action writes anything to disk. On failure:
