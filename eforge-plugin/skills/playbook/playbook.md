...
1. **Check the capability through the playbook run response**: Call `mcp__eforge__eforge_extension_contribution { action: "invoke", kind: "command", id: "eforge-playbooks:run-playbook", input: { name: "<name>" } }`. This does not enqueue planning-mode playbooks. It checks the required `eforge.plan.planning-workstation` capability.
