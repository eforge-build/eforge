...
**Compiled-build resume**: On confirmation for resume, call `mcp__eforge__eforge_resume_build` with `{ prdId }`. The daemon spawns a background build agent and returns `{ sessionId, pid }`. Report:
