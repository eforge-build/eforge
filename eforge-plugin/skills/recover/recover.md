...
If `recoveryOptions` contains `kind: "compile-scope-context"`, treat it as non-mutating guidance: summarize its action, eligibility, source/failure kind, reason, and any bounded decomposition evidence summary, but do not map it to `mcp__eforge__eforge_apply_recovery` or `mcp__eforge__eforge_continue_repair`.
