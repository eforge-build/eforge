...
This skill does not create backend profiles. Before gathering team-wide settings, confirm a backend profile is already active by calling `mcp__eforge__eforge_backend` with `{ action: "show" }`. Inspect the response:
...
Modify the config based on the user's requests. Present the updated content before writing. If the change involves a model ID anywhere (`agents.models.*`, `agents.model`, per-role `model`), first resolve the active backend by calling `mcp__eforge__eforge_backend` with `{ action: "show" }`, then call `mcp__eforge__eforge_models` with `{ action: "list", backend: "<resolved-backend>" }` (and `provider` for Pi) to fetch the live list. Pick from that list; do not suggest model IDs from memory.
...
|-------|---------|----------------|
| Init | `/eforge:init` | No eforge config found in the project, or no active backend profile |
