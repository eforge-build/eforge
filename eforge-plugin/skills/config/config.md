...
12. **Pi backend** (conditional - only if user chose `backend: pi` in step 1) - `provider` (for example `openrouter`, `anthropic`, or OAuth-backed providers like `openai-codex`), `thinkingLevel` (`off`/`medium`/`high`), `extensions` (auto-discover from `.pi/extensions/`), `compaction` (context compaction threshold), and `retry` config. Authentication usually comes from env vars or `~/.pi/agent/auth.json`; model selection still uses `agents.model` or `agents.models.*`, not `pi.model`. Note: experimental and untested.
...
# pi:
#   provider: openrouter               # LLM provider, e.g. openrouter / anthropic / openai-codex
