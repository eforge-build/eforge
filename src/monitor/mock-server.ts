...
insertEvent(RUN1_ID, { type: 'plan:profile', profileName: 'errand', rationale: 'Single endpoint addition with no dependencies — errand profile fits best', config: { description: 'Small, self-contained changes.', compile: ['planner', 'plan-review-cycle'] } } as unknown as EforgeEvent, 5500);
...
insertEvent(RUN3_ID, { type: 'plan:profile', profileName: 'errand', rationale: 'Single middleware addition — errand profile for low-risk single-area change', config: { description: 'Small, self-contained changes.', compile: ['planner', 'plan-review-cycle'] } } as unknown as EforgeEvent, 610500);
insertEvent(RUN3_ID, {
  type: 'plan:complete',
  plans: [{
...
insertEvent(RUN5_ID, { type: 'plan:profile', profileName: 'errand', rationale: 'Caching layer addition to existing endpoints — errand profile fits', config: { description: 'Small, self-contained changes.', compile: ['planner', 'plan-review-cycle'] } } as unknown as EforgeEvent, 755500);
insertEvent(RUN5_ID, {
  type: 'plan:complete',
  plans: [{
    id: 'plan-01-caching',
...
db.insertEvent({ runId: RUN4_ID, type: 'plan:profile', data: JSON.stringify({ type: 'plan:profile', profileName: 'excursion', rationale: 'Pagination across routes, query parsing, and tests — excursion profile for multi-file feature work', config: { description: 'Multi-file feature work or refactors.', compile: ['planner', 'plan-review-cycle'] } }), timestamp: runTs(8500) });
