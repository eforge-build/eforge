...
- **Runtime queue files** (`.eforge/queue/`) — normalized PRDs waiting for daemon processing. Enqueue stores a validated, hidden canonical acceptance-criteria inventory with stable `ac-###` IDs in each queued PRD. Queue files are gitignored, ephemeral runtime state only; queue mutations are filesystem operations that produce no git commits.
