...
Plan a feature interactively, then hand it off to eforge with `/eforge:build`. A daemon picks up the plan and runs planning, building, blind review, and validation autonomously. A web monitor (port assigned deterministically per project in the 4567-4667 range) tracks progress, cost, and token usage in real time. The active monitoring dashboard (Eforge Console) is served at `/console/`; the legacy monitor dashboard remains at `/` on the same port.
...
- **Runtime queue files** (`.eforge/queue/`) — normalized PRDs waiting for daemon processing. Also gitignored, ephemeral runtime state only; queue mutations are filesystem operations that produce no git commits.
