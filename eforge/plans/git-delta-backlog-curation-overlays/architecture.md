...
Current implementation already has bounded curation source assembly, shipped-evidence candidates, two-step curation apply, recommendation freshness sidecars, same-draft closed-target filtering, and a workstation preview. Gaps remain: there is no accepted-analysis git baseline sidecar, no `gitDelta` source section, shipped evidence always scans recent `HEAD` history instead of baseline ranges, prefix validation is not enforced for closed curation patches, preview/apply share only a limited closed-target filter rather than a full prospective projection, stale submitted session-plan trace rows can still appear active, and the workstation has no git-delta diagnostics/effective overlay projection.
