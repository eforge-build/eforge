...
- `eforge/extensions/eforge-plan/backlog-query-actions.ts` — remove the legacy in-memory `search-items` schema/handler and route `search-items` wiring to the FTS search action; keep non-search projections owned by `projections-lifecycle` unchanged `[region: fts-search-bounded-actions, SearchItemsInputSchema/SearchItemsOutputSchema/search-items handler and snippets/counts output]`.
