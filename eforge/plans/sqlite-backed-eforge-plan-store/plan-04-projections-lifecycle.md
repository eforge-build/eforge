...
name: Implement SQL-derived board lanes, item/session lifecycle, recommendation
  actionability, associated plan/build links, active build linkage, and compact
  projections.
...
- `eforge/extensions/eforge-plan/backlog-query-actions.ts` — replace `get-item`, `get-epic`, and `list-board-compact` loading/mapping with SQL projection calls; add additive compact item fields to the non-search schemas. `search-items` schema/handler lives in `eforge/extensions/eforge-plan/search/` for `fts-search-bounded-actions` `[region: projections-lifecycle, GetItem/GetEpic/ListBoardCompact SQL projection loading and compact item/epic mapping]`.
