...
agents:
  builder:
    ...
    rationale: "Cross-package coordinated rename touching ~18 files. Mechanical but
      ...
      three parallel scope groups (types/engine, MCP tools, skills+tests) so
      each shard has tight focus while the merged result must compile
      coherently."
    shards:
...
- `eforge-plugin/skills/profile-new/profile-new.md` — Add a one-line note at the top of the workflow about multiple agentRuntimes entries. L8 description: "selects a backend kind" → "selects a harness". Step 2 heading "Pick the backend kind" → "Pick the harness". Body "Which backend?" → "Which harness?". Smart-default rules reworded ("Names starting with `pi-` default to harness `pi`", etc.). Step 3 (provider): `eforge_models` example uses `harness: "pi"`. Step 4 (models): `harness: "claude-sdk"` / `harness: "pi"`. "All backends" bullet at L76: "All harnesses". Step 6 synthesis snippet (L87): top-level `backend:` field → `harness:`. Sample YAML at L105: `backend: pi` → `harness: pi`. Step 7 `eforge_profile create` body at L131: `harness:` not `backend:`.
- `eforge-plugin/skills/profile/profile.md` — L23 shape comment: `{ active, source, resolved: { backend, profile } }` → `{ active, source, resolved: { harness, profile } }`. L27 bullet: "Resolved backend" → "Resolved harness", `{resolved.backend}` → `{resolved.harness}`. L47, L49 success message: "resolved backend" → "resolved harness", `{resolved.backend}` → `{resolved.harness}`.
- `eforge-plugin/skills/config/config.md` — L32 `resolved.backend` → `resolved.harness`. L35 "resolved backend kind" → "resolved harness", `resolved.backend` → `resolved.harness`. L143 inline comment "resolved backend" → "resolved harness". L95 inline `eforge_models` call: `backend: "<resolved-backend>"` → `harness: "<resolved-harness>"`. References to the **legacy YAML key** `backend:` in L49, L71, L91, L117, L229 (warning users that `backend:` does not belong at the top of `config.yaml`) stay as literal references but reword surrounding prose where it talks about "the backend" as a concept (e.g., "switching backend kind" → "switching harness").
- `packages/pi-eforge/skills/eforge-init/SKILL.md` — Mirror Plugin init.md changes. Reword "Since Pi is the backend" → "Since this skill targets the Pi harness". Sub-bullet 1 of Step 1.5 uses `harness: "pi"`. Add the same mixed-harness hand-off pointer at Step 4. Migrate paragraph at L50: keep the literal `backend:` legacy field reference, reword the surrounding sentence.
- `packages/pi-eforge/skills/eforge-profile-new/SKILL.md` — Mirror Plugin profile-new.md changes one-for-one (same line numbers in the Pi parity copy).
- `packages/pi-eforge/skills/eforge-profile/SKILL.md` — Mirror Plugin profile.md changes (L26, L30, L50, L52).
- `packages/pi-eforge/skills/eforge-config/SKILL.md` — Mirror Plugin config.md changes (L34, L37, L97, L145; preserve legacy `backend:` literal references at L51, L73, L93, L119, L231 while rewording surrounding prose).
- `test/config-backend-profile.test.ts` — Update imports at L11-12 from `createBackendProfile, deleteBackendProfile` → `createAgentRuntimeProfile, deleteAgentRuntimeProfile`. Update `describe` block titles at L227, L290, L611, L648, L840 to use the new function names. Update all call sites (L241, L251, L264, L266, L271, L272, L285, L306, L314, L320, L636, L678, L689, L869, L879, L888, L896). Do **not** rename the test file itself — preserves git history.
...
- [ ] `eforge_profile` MCP tool accepts `{ action: "create", name: "x", harness: "claude-sdk" }` and rejects `{ ..., backend: "claude-sdk" }` with a Zod error mentioning `harness` (verified via the existing schema test or a new line in `test/config-backend-profile.test.ts`).
- [ ] `eforge_models` MCP tool accepts `{ action: "providers", harness: "pi" }` and rejects `{ ..., backend: "pi" }`.
