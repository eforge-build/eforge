...
export function normalizeIncludes(include: RunPlanningStoreImportOptions['include']): PlanningStoreImportInclude[] { const allowed = new Set(PLANNING_STORE_IMPORT_INCLUDES); const values = include?.length ? include : PLANNING_STORE_IMPORT_INCLUDES; return PLANNING_STORE_IMPORT_INCLUDES.filter((v) => values.includes(v) && allowed.has(v)); }
