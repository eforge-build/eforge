...
import { MAX_IMPORT_DIAGNOSTIC_LIMIT, PLANNING_STORE_IMPORT_INCLUDES } from './types.js';
...
export const ImportPlanningStoreInputSchema = Type.Object({ dryRun: Type.Optional(Type.Boolean()), replaceExisting: Type.Optional(Type.Boolean()), include: Type.Optional(Type.Array(IncludeSchema, { minItems: 1, uniqueItems: true })), diagnosticLimit: Type.Optional(Type.Integer({ minimum: 0, maximum: MAX_IMPORT_DIAGNOSTIC_LIMIT })) }, { additionalProperties: false });
