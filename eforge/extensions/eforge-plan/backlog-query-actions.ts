...
export const backlogQueryActions = [
  defineExtensionAction({
    ...
    description: 'Read one backlog item with compact dependency/dependent summaries and Markdown sections without listing the whole board.',
  ...
  defineExtensionAction({
    ...
    description: 'Return bounded kanban item summaries, lane counts, and epic counts without full item bodies, traces, or lifecycle links.',
...
async function listBoardCompact(cwd: string, input: ListBoardCompactInput) {
  const [items, epics, cards] = await loadBoardCards(cwd, input.includeArchive, input.epic);
  const filtered = input.lane ? cards.all.filter((card) => card.lane === input.lane) : cards.all;
  const page = paginate(filtered, input);
  return {
    schemaVersion: 1 as const,
    items: page.entries.map(compactItem),
    ...
    limit: page.limit,
    offset: page.offset,
    lanes: cards.lanes.map((lane) => ({ lane: lane.lane, title: lane.title, count: lane.items.length })),
