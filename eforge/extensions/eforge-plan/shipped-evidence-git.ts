...
export async function collectGitHistoryRecords(cwd: string, caps: Partial<ShippedEvidenceCaps> = {}): Promise<GitHistoryCollection> {
  const limits = normalizeShippedEvidenceCaps(caps);
  const diagnostics: ShippedEvidenceDiagnostic[] = [];
  const inside = await runGit(cwd, ['rev-parse', '--is-inside-work-tree'], limits);
  if (!inside.ok) {
  ...
  for (const entry of log.stdout.split(LOG_SEPARATOR).map((value) => value.trim()).filter(Boolean)) {
    ...
    const changedPaths = await collectChangedPaths(cwd, hash, limits, diagnostics);
    const text = `${subject}\n${body}`;
    records.push({
      source: 'git-history',
      hash,
...
export async function collectGitFileExcerpts(input: {
  ...
  for (const path of input.record.changedPaths.slice(0, limits.changedPathCount)) {
    ...
    const shown = await runGit(input.cwd, ['show', `${input.record.hash}:${path}`], limits, Math.max(64 * 1024, limits.excerptBytes * 16));
    if (!shown.ok) {
...
async function collectChangedPaths(cwd: string, hash: string, caps: ShippedEvidenceCaps, diagnostics: ShippedEvidenceDiagnostic[]): Promise<string[]> {
  const result = await runGit(cwd, ['diff-tree', '--no-commit-id', '--name-only', '-r', '-m', hash], caps);
  if (!result.ok) {
...
async function runGit(cwd: string, args: readonly string[], caps: ShippedEvidenceCaps, maxBuffer = 1024 * 1024): Promise<{ ok: true; stdout: string } | { ok: false; detail: string }> {
  try {
    const { stdout, stderr } = await execFile('git', [...args], { cwd, timeout: caps.subprocessTimeoutMs, maxBuffer });
