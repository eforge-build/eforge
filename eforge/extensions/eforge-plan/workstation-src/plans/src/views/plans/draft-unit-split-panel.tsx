...
export function DraftUnitSplitPanel({ unit, titles, onAdvise, onSplit, onClose, onOpenUnit }: DraftUnitSplitPanelProps) {
  ...
  return (
    <div className="grid gap-2 rounded-md border border-dashed border-border p-3" aria-label="Split draft unit">
      <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
        <Scissors className="h-3.5 w-3.5" /> Split into a new unit
        <Button variant="ghost" size="icon-xs" className="ml-auto" aria-label="Close split panel" disabled={busy} onClick={onClose}><X className="h-3.5 w-3.5" /></Button>
      </div>
        ...
        {unit.items.map((item) => (
          <li key={item.itemId}>
            <label className="flex items-center gap-2 rounded border border-border p-1.5 text-xs">
              <Checkbox disabled={busy} checked={selected.has(item.itemId)} onChange={() => toggle(item.itemId)} />
      ...
      <div className="flex items-center gap-2">
        <Button size="sm" disabled={busy || !validSubset || !title.trim()} onClick={split} title={!validSubset ? 'Keep at least one item on each side.' : 'Split off the selected items.'}>
          {busy ? <Spinner /> : <Scissors className="h-4 w-4" />} Split off {peelIds.length || ''} {peelIds.length === 1 ? 'item' : 'items'}
        </Button>
