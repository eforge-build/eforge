...
- `analyze-all-backlog` example input: `{}`; starts or reuses a daemon-owned backlog curation task for the current visible open backlog fingerprint. The task requests both `backlogCurationDraft` and `recommendations` output and does not enqueue builds. This is the primary workstation path for refreshing recommendations because normal confirmed curation applies the generated recommendation model against the post-curation backlog state; curation-only apply intentionally discards generated recommendations.
