...
- `analyze-all-backlog` example input: `{}`; starts or reuses a daemon-owned backlog curation task for the current visible open backlog fingerprint. The task requests both `backlogCurationDraft` and `recommendations` output and does not enqueue builds. Its curation source includes bounded git/PR history shipped evidence alongside lifecycle trace evidence; optional PR enrichment uses the GitHub CLI (`gh pr view`) when available and authenticated, may perform network access, and is fail-closed, so unavailable `gh`, unauthenticated access, timeouts, or command failures leave bounded git-only candidate evidence in place instead of failing the task. This is the primary workstation path for refreshing recommendations because normal confirmed curation applies the generated recommendation model against the post-curation backlog state; curation-only apply intentionally discards generated recommendations.
- `refresh-recommendations` example input: `{}`; starts or reuses a daemon-owned recommendation-only planning task for the current open backlog fingerprint. This action remains available as a lower-level/API compatibility path, but the workstation favors `analyze-all-backlog` so users do not choose between curation and recommendation refresh.
...
- `list-plan-revision-sessions` example input: `{ "includePlan": true }`; lists persisted revision threads joined to owner-scoped daemon task records.
- `get-plan-revision-session` example input: `{ "session": "2026-06-05-add-import-preview" }`; returns one revision thread by session or `threadId`.
- `start-plan-revision-turn` example input: `{ "session": "2026-06-05-add-import-preview", "message": "Tighten the scope section." }`; starts one read-only `eforge-plan.planning-draft` task with `requestedOutputSections: ["planRevisionTurn"]` against the current flat plan fingerprint.
- `retry-plan-revision-turn` example input: `{ "session": "2026-06-05-add-import-preview", "turnId": "turn_123" }`; retries a linked turn, or redrafts a completed needs-input turn when `answers` or `steering` are provided.
...
- `apply-plan-revision-turn` example input: `{ "session": "2026-06-05-add-import-preview", "turnId": "turn_123", "sections": ["scope"], "previewAcknowledged": true, "confirmApply": true }`; applies only selected structured section patches after explicit apply confirmation when the current full-plan fingerprint still matches the stored base fingerprint, or when drift is limited to sections already applied from the same turn. Stale base/section drift returns `kind: "stale"`; answer-only, mismatched, already-applied, or invalid selections return `kind: "not-applicable"` without writing session-plan sections.
...
- `.eforge/storage/extensions/eforge-plan/plan-revisions/index.json` stores private revision-session threads for existing flat session plans, including turn/task links, base fingerprints, section hashes, retry/redraft linkage, and applied section metadata.

The extension never reads or writes legacy `.backlog/recommendations.json`; recommendation state lives only in private extension storage. Backlog records, trace sidecars, recommendation files, planning task workflow records, and plan revision session records are private extension storage; session plans are public build inputs under `.eforge/session-plans/`.
...
Recommendation model writes validate references before changing storage: `put-recommendations`, `apply-planner-result`, `apply-planning-agent-task-result`, and confirmed `applyBacklogCurationDraft` output reject unknown `itemId`/`epicIds` references, closed item/epic recommendation references, and empty safe-parallelizable group `itemIds`. Generated recommendations from a backlog curation draft are first filtered through the same-draft status overlay so item ids proposed as shipped by that draft are removed before preview validation and apply validation. Backlog curation task list entries can include preview-time generated recommendation validation so the workstation can show invalid generated recommendation references before apply; backend apply repeats validation and remains authoritative. Validation, reference, and curation precondition failures leave the existing `current.json` and status sidecar unchanged; there are no partial writes. Successful writes update `current.json` first, then derive freshness from the applied source fingerprint. When curation output includes generated recommendations, confirmed `applyBacklogCurationDraft` writes private backlog records first, writes the private recommendation model only after validation succeeds, and records freshness against the post-apply/post-curation backlog fingerprint with `lastRefreshedBy: "apply-backlog-curation-draft"`. If that fingerprint has drifted by the time the model is applied, `apply-planning-agent-task-result` can return stale status with a `source-fingerprint-drift` reason instead of fresh. Fresh status records `lastRefreshedBy` as `put-recommendations`, `apply-planner-result`, `apply-planning-agent-task-result`, or `apply-backlog-curation-draft`, depending on the action that applied the model.
...
| `analyze-all-backlog` | Start or reuse a daemon-owned backlog curation planning task for the current visible open backlog fingerprint. It records a durable workflow entry with purpose `backlog-curation`, requests `backlogCurationDraft` plus `recommendations`, includes bounded git/PR shipped evidence alongside lifecycle trace evidence, and does not enqueue builds. Optional PR enrichment fails closed, leaving bounded candidate evidence in place instead of failing the task. Normal confirmed apply writes curation changes first, then refreshes recommendations from the post-curation backlog state, so this is the primary workstation refresh path; curation-only apply discards generated recommendations when explicitly requested. | `local-read`, `local-write`, `daemon-state`, `network` |
| `refresh-recommendations` | Start or reuse a daemon-owned recommendation-only planning task for the current source fingerprint. It records a durable workflow entry with purpose `recommendation-refresh` and does not apply generated output automatically. This lower-level action remains registered but is not exposed as the primary workstation control. | `local-read`, `local-write`, `daemon-state` |
...
| `list-planning-agent-tasks` | Project the durable planning task workflow index and join owner-scoped daemon task records for discovery, polling, retry, and redraft across reloads, including backlog curation preview validation details when available. | `local-read` |
...
| `get-plan-revision-session` | Return one private revision thread by target session or thread id, joined to daemon task records. | `local-read` |
| `start-plan-revision-turn` | Start one read-only `eforge-plan.planning-draft` task for a user revision message, requesting only `planRevisionTurn` output and recording the base plan fingerprint. | `local-read`, `local-write`, `daemon-state` |
| `retry-plan-revision-turn` | Retry a linked revision turn or redraft a completed needs-input turn from clarification answers while preserving parent linkage. | `local-read`, `local-write`, `daemon-state` |
...
| `apply-plan-revision-turn` | Apply selected structured section patches from a completed revision turn after confirmation and full-plan fingerprint validation; stale or not-applicable turns write no sections. | `local-read`, `local-write` |
