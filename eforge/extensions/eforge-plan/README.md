...
- `update-roadmap-state` example input: `{ "localFocusContent": "# Focus\n\nShip local roadmaps.\n", "expectedLocalFocusSha256": "..." }`; writes only private local-focus roadmap content and/or shared-source configuration under extension storage. Configured shared project files are read-only context and are not rewritten.
...
- `.eforge/storage/extensions/eforge-plan/roadmaps/config.json` stores configured shared roadmap source metadata. Shared project roadmap files remain read-only context; conventional files such as `docs/roadmap.md` may be discovered as non-canonical shared context when not configured.
...
| `update-roadmap-state` | Update private local-focus roadmap content and/or configured shared-source metadata. It validates configured paths stay within the project and never writes the configured shared project files themselves. | `local-read`, `local-write` |
...
| `apply-plan-revision-turn` | Write all structured section patches from a completed revision turn, resolve referenced open annotations, and refresh readiness; apply is idempotent and not-applicable turns write no sections or annotation resolution. | `local-read`, `local-write` |
