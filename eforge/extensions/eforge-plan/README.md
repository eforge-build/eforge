...
- `.eforge/storage/extensions/eforge-plan/planning-tasks/index.json` stores the extension-owned durable planning workflow index used by the "Plan with AI" monitor for AI planning task discovery, polling, retry, and redraft context across reloads.
...
| `apply-planning-agent-task-result` | Apply selected output from a completed planning-draft task through recommendation storage, handoff promotion helpers, session-plan section adapters, and `applySessionPlanCreationDraft` for ready creation drafts. | `local-write` |
...
- **Backlog** renders the derived kanban with Lane / Epic / Recommended grouping, status filters (`all`, `ready`, `blocked`, `review`, `closed`), free-text and epic filters, and a next-up recommendations rail with blocked chains and rationale. Selecting ready items exposes a single **Promote to a build plan** action that starts AI session-plan generation for the ready subset of the selection (blocked, closed, and non-ready items are excluded, and the action is disabled when no ready items remain); recommendation cards and groups start the same AI planning workflow by item ids or recommendation ref. There is no deterministic workstation promotion path and no prompt-input box: the AI promotion derives its goal from the selection. Recommendations stay in the AI workflow rather than calling `promote-selection`.
