...
- `retry-plan-revision-turn` example input: `{ "session": "2026-06-05-add-import-preview", "taskId": "task_123" }`; retries a linked turn, or redrafts a completed needs-input turn when `answers` or `steering` are provided.
- `cancel-plan-revision-turn` example input: `{ "session": "2026-06-05-add-import-preview", "taskId": "task_123", "reason": "Superseded." }`; cancels the linked daemon task.
- `apply-plan-revision-turn` example input: `{ "session": "2026-06-05-add-import-preview", "taskId": "task_123", "sections": ["scope"], "previewAcknowledged": true, "confirmApply": true }`; applies only selected structured section patches when the current full-plan fingerprint still matches the stored base fingerprint. Stale, answer-only, mismatched, or invalid selections return `kind: "stale"` or `kind: "not-applicable"` without writing session-plan sections.
