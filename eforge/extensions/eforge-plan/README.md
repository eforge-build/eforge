...
- `start-planning-agent-task` example input: `{ "userGoal": "Find the safest next import-preview work", "itemIds": ["add-import-preview"], "includeRoadmap": true }`; prepares bounded planner context, then starts a daemon-owned `eforge-plan.planning-draft` task.
...
| `start-planning-agent-task` | Prepare planner context, then ask the daemon-owned agent task service to run one `eforge-plan.planning-draft` task for the supplied user goal. | `local-read`, `local-write`, `daemon-state` |
...
| `apply-planning-agent-task-result` | Apply selected output from a completed planning-draft task through recommendation storage, handoff promotion helpers, and session-plan section adapters. | `local-write` |
...
The workstation can browse backlog-derived board data, recommendation summary data, epics, flat session plans, and session plan sets; create or edit session plans; update metadata and selected dimensions; run readiness checks; mark ready plans; promote a recommended item, recommended group, epic, or user-selected one-or-more-item set through `promote-selection`; run a single-shot **Plan with AI** task; preview generated recommendations, handoff drafts, and session-plan draft sections; apply only selected generated output after an explicit in-app confirmation; and perform source-path handoff after an explicit confirmation. All reads and mutations go through `window.eforge.invokeAction` and the workstation manifest's `allowedActions` list.
...
The AI planning MVP is intentionally single-shot and limited to read-only MVP tools while the task is running: the workstation can start, poll, cancel, and preview one planning task, but it does not provide multi-turn chat, autonomous backlog draining, or automatic application of generated content.
