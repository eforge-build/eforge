...
export interface ProjectionItemRow { id: string; title: string; body: string; userStatus: UserStatus; priority?: string; source?: string; createdAt?: string; updatedAt?: string; epicId?: string; epicRef?: string; bodySha256?: string; recordSha256?: string; frontmatter: Record<string, unknown>; tags: string[] }
...
export function rowToProjectionItem(store: EforgePlanStore, row: Record<string, unknown>): ProjectionItemRow { const id = row.id as string; return { id, title: row.title as string, body: row.body as string, userStatus: row.user_status as UserStatus, priority: optionalString(row.priority), source: optionalString(row.source), createdAt: optionalString(row.created_at), updatedAt: optionalString(row.updated_at), epicId: optionalString(row.epic_id) ?? optionalString(row.epic_ref), epicRef: optionalString(row.epic_ref), bodySha256: optionalString(row.body_sha256), recordSha256: optionalString(row.record_sha256), frontmatter: parseJsonColumn('backlog_items','frontmatter_json', row.frontmatter_json, {}) as Record<string, unknown>, tags: tags(store, 'backlog_item_tags', 'item_id', id) }; }
