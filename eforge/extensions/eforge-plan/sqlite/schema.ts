...
CREATE TABLE IF NOT EXISTS import_runs (run_id TEXT PRIMARY KEY, dry_run INTEGER NOT NULL DEFAULT 0 CHECK (dry_run IN (0,1)), applied INTEGER NOT NULL DEFAULT 0 CHECK (applied IN (0,1)), replaced_existing INTEGER NOT NULL DEFAULT 0 CHECK (replaced_existing IN (0,1)), started_at TEXT, finished_at TEXT, counts_json TEXT, summary_json TEXT, verbose_report_json TEXT, verbose_report_prunable INTEGER NOT NULL DEFAULT 1 CHECK (verbose_report_prunable IN (0,1)));
CREATE TABLE IF NOT EXISTS import_diagnostics (diagnostic_id TEXT PRIMARY KEY, run_id TEXT NOT NULL REFERENCES import_runs(run_id) ON DELETE CASCADE, severity TEXT NOT NULL, code TEXT NOT NULL, message TEXT NOT NULL, ref TEXT, path TEXT, details_json TEXT);
CREATE INDEX IF NOT EXISTS idx_import_diagnostics_run ON import_diagnostics(run_id); CREATE INDEX IF NOT EXISTS idx_import_diagnostics_severity ON import_diagnostics(severity);
