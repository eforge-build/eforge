...
A planning-draft result is either a ready result that carries at least one applicable output section (including an optional `sessionPlanCreationDraft` when a session plan is requested) or a `needs-input` decision that carries structured clarification questions and a rationale instead of a session-plan file output. While a task runs, the agent may report telemetry-only `sectionProgress` (current, covered, and remaining sections) through a read-only progress tool. The daemon sanitizes and length-caps that progress before persisting it on the running record and emitting it on sanitized progress events; section progress is advisory telemetry only and never determines readiness or apply eligibility.
