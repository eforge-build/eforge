...
The **console panel** in the monitor UI exposes four lower tabs: `Log` (raw event stream), `Changes` (per-plan file diffs), `Graph` (plan dependency graph), and `Plan` (planner decisions). The `Plan` tab renders three sections from the event log - Classification (mode badge from `planning:pipeline`), Pipeline (compile/build/review config), and Plans (per-plan build stage breakdown and review profile from `planning:complete`'s `planConfigs`). Orchestration data is sourced exclusively from the `planning:complete` event payload via the reducer (`runState.earlyOrchestration`) — there is no separate endpoint fetch, so per-plan stage breakdowns remain accurate for completed sessions after worktrees have been cleaned.
