...
| Stage | Description |
|-------|-------------|
| `planner` | The bounded planner compiler explores the codebase through bounded exploration/atom/reducer agents, normalizes the model proposal through one deterministic executable representation, then synthesizes plan files, `orchestration.yaml`, `architecture.md`, `acceptance-coverage.md`, and `compiler-diagnostics.json`. Normalization enforces module/context ceilings, coverage, dependencies, exclusive file ownership, compatible docs/test stages, and one test author without creating extra plans; diagnostics retain proposed intent, fallback and normalization reasons, and safety escalations. Repository exploration reports a structured outcome (`completed`, `needs-rescope`, `budget-exhausted`, or `ambiguous`); budget exhaustion enters a submit-only grace path and diagnostics retain unresolved needs, shared reasons, attempted queries, candidate paths, unknown-id drops, and tool-use count. A degraded outcome on a risky collapsed root atom enters a bounded adaptive rescope loop: deterministic directives split the atom graph (`reason: 'rescope-split'`), exploration reruns per unresolved scope under a cross-run tool-use ledger, and the loop's state lands in the `rescope` section of `compiler-diagnostics.json`. Exhausted rescoping with critical (interface/entrypoint) needs still unresolved fails the compile closed via `AdaptiveRescopeFailClosedError` - a new failure mode where a previously degraded-but-proceeding compile now stops - and persists its state to `rescope-fail-closed.json` in the plan-set directory. Tool validation failures return bounded diagnostics with schema path, expected/received type summaries, payload byte count, SHA-256 hash, omitted-byte/truncation metadata, and compact excerpts rather than echoing raw submitted arguments. The `AgentHarness` translates bare tool names into the harness-visible identifier (Claude SDK prefixes `mcp__eforge_engine__`; Pi uses the bare name). |
...
There is no compile-time scope classification: every compile runs the constant `[planner, planning-quality-review-cycle]` pipeline. The bounded planner compiler adapts to input size through deterministic inventory chunking - trivial sources take the single-atom passthrough fast path, and oversized sources fan out into bounded planning units - so no agent decides which pipeline to run. Reducers receive compiler-recorded context, criterion, and subsystem ceilings that every proposed module must fit. An oversized reducer submission - including the digest rebuilt from its module candidates that parent reducers consume - is rejected with exact budget diagnostics and may be corrected within the same bounded model run; deterministic code never silently invents replacement boundaries or drops coverage. Final proposal normalization repeats the same shared budget calculation and fails closed if an invalid boundary remains. Per-plan build/review defaults come from the compiler's deterministic pipeline derivation, audited by the planning-quality gate. Compile scope/context recovery reports bounded-decomposition or manual-reduce-scope guidance with decomposition evidence when planner-stage evidence shows that is the bounded recovery path. See [config.md](config.md) for tier and role configuration.
...
Planning-quality review additionally supports typed structural simplifications for merging cohesive non-residue plans, removing redundant non-core stages, and reducing review depth. The engine applies each structural submission as one deterministic candidate transaction, rechecks compiler-recorded context/criterion/subsystem budgets, dependency acyclicity, test ownership, stage compatibility, and review floors, then regenerates only the artifacts the fixes actually change (touched plans, orchestration, and, on merges, architecture); acceptance coverage and untouched plans are preserved unchanged. The evaluator must accept or reject the generated path group as a unit; deterministic compiler cohesion validation runs again before planning completes.
...
Review-fixer retry context separates rejected verifier issues from unresolved verifier issues so a recovery attempt can avoid broad fixes the evaluator rejected while still targeting blockers that remain unresolved.
