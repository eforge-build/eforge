...
Each build phase gets its own agent role: formatter, planner, builder, reviewer, evaluator, fixer, doc-updater, validation-fixer. Agent runners use an `AgentBackend` interface - all LLM interaction is isolated behind provider-specific adapters (`ClaudeSDKBackend` for Anthropic's SDK, `PiBackend` for pi-mono's multi-provider support), making the engine provider-swappable.
