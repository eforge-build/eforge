...
describe('profile-commands.ts imports toolbelt modules (plan-01-pi-toolbelt-preset-ux)', () => {
  ...
  it('calls configValidate before profileCreate when preset mutations are pending', () => {
    // When a preset is applied (not skipped), config validation must run before profile creation
    expect(source).toContain('configValidate');
    const configValidateIdx = source.indexOf('configValidate');
    const profileCreateIdx = source.lastIndexOf('profileCreate');
    expect(configValidateIdx).toBeGreaterThan(-1);
    expect(configValidateIdx).toBeLessThan(profileCreateIdx);
  });
});
