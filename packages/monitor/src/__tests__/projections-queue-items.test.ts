...
function recoverySidecar(prdId: string, applied: unknown): Record<string, unknown> {
...
describe('queue item projections', () => {
  ...
  it('keeps sync and async loaders in parity across statuses, sidecars, and dependsOn filtering', async () => {
    ...
    writeFileSync(join(queue, 'failed', 'accepted-failed.recovery.json'), JSON.stringify(recoverySidecar('accepted-failed', { action: 'accepted-success', acceptedAt: '2025-01-03T00:00:00.000Z', reasonCategory: 'other', reason: 'ok', cleanup: { status: 'noop' }, landing: { action: 'pr', status: 'failed', branch: 'eforge/y', reason: 'nope' }, dependents: { unblocked: [], remainedBlocked: [], notFound: [] } })));
