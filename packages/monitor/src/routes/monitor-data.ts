...
import { projectQueueForContext, projectRunsForContext, projectSessionMetadataForContext } from '../projections/monitor-state.js';
...
export function createMonitorDataRoutes(context: MonitorContext): RouteDefinition[] {
  ...
  return [
    defineRoute({ routeKey: 'queue', method: 'GET', pattern: API_ROUTES.queue, security: readSecurity, async handler(ctx) {
      sendJson(ctx.res, await projectQueueForContext(context));
    } }),
    defineRoute({ routeKey: 'sessionMetadata', method: 'GET', pattern: API_ROUTES.sessionMetadata, security: readSecurity, handler: (ctx) => sendJson(ctx.res, projectSessionMetadataForContext(context)) }),
    defineRoute({ routeKey: 'runs', method: 'GET', pattern: API_ROUTES.runs, security: readSecurity, handler: (ctx) => sendJson(ctx.res, projectRunsForContext(context)) }),
