...
Call the `eforge_backend` tool with `{ action: "show" }`.
...
Then call `eforge_backend` with `{ action: "list" }` to show the user what other profiles are available, rendering a table with the following columns:
...
Call `eforge_backend` with `{ action: "use", name: "<arg>" }`. To set the active profile at user scope instead of project scope, pass `scope: "user"`: `{ action: "use", name: "<arg>", scope: "user" }`.
