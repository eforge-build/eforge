...
Then, optionally, call `eforge_backend` with `{ action: "list" }` to show the user what other profiles are available, rendering a short table of profile names and marking the active one with `●`.

If no profiles exist, suggest `/eforge:backend:new` to create one.

### Step 3: Switch mode

Call `eforge_backend` with `{ action: "use", name: "<arg>" }`.
