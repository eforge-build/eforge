...
| PRD failed after compile — feature branch exists with partial work | Use `eforge_apply_recovery` with a `split` verdict to enqueue a continuation successor; use `eforge_resume_build` only when you want to rerun the original compiled artifacts |
