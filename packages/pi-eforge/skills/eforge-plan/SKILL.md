...
1. If `--session <session-id>` is provided, call `eforge_session_plan { action: 'show', session: '<session-id>', open: true }` directly and skip the active-session menu. Otherwise, call `eforge_session_plan { action: 'list-active' }` to discover active sessions.
...
1. If `--playbook <name>` is provided, use that exact playbook name and skip the playbook menu. Otherwise, call `eforge_playbook { action: 'list' }`.
...
6.5. **Synthesize implementation handoff**: Before asking for a topic, convert findings into an implementation-ready handoff:
...
7. Ask the user for a topic that describes the implementation change to build — not the investigation already performed (e.g., "Reduce complexity in event parser" rather than "Audit complexity hotspots"). If the synthesized targets make the topic obvious, suggest it and allow override.
...
11. Proceed to Step 2 (Gather Context) — the synthesized implementation handoff from step 6.5 is the authoritative starting context. Step 2 resumes from the synthesized handoff without re-running covered investigation.
...
4. Call `eforge_session_plan { action: 'set-section', session, dimension: '{dimension-name}', content: '{content}' }` to record the dimension content.
...
- **On resume**: call `eforge_session_plan { action: 'migrate-legacy', session }` — the tool converts covered dimensions to body entries, uncovered ones to `required_dimensions` entries, adds `assumptions-and-validation` as a required dimension, and rewrites frontmatter to the new shape. Then reload via `{ action: 'show', session }` to continue with the migrated data.
