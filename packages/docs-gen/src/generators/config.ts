...
import { z } from 'zod';
import { DEFAULT_PLANNING_DECOMPOSITION_CONFIG, eforgeConfigSchema } from '@eforge-build/engine/config';
...
export async function generateConfig(opts: {
  ...
  lines.push('## Compile');
  lines.push('');
  lines.push('`compile` tunes context-managed planning units for overflow-risk compile inputs. Ordinary direct planning does not use these limits. All `compile.planningUnit*` values are positive integers when set.');
  lines.push('');
  lines.push('| Field | Default | Description |');
  lines.push('|-------|---------|-------------|');
  lines.push(`| \`compile.planningUnitParallelism\` | \`${DEFAULT_PLANNING_DECOMPOSITION_CONFIG.planningUnitParallelism}\` | Maximum planning units processed concurrently. |`);
  lines.push(`| \`compile.planningUnitMaxDepth\` | \`${DEFAULT_PLANNING_DECOMPOSITION_CONFIG.planningUnitMaxDepth}\` | Maximum recursive split depth for planning units. |`);
  lines.push(`| \`compile.planningUnitMaxPromptSourceBytes\` | \`${DEFAULT_PLANNING_DECOMPOSITION_CONFIG.planningUnitMaxPromptSourceBytes}\` | Maximum source bytes packed into a single planning-unit prompt. |`);
  lines.push(`| \`compile.planningUnitMaxPromptBytes\` | \`${DEFAULT_PLANNING_DECOMPOSITION_CONFIG.planningUnitMaxPromptBytes}\` | Maximum total prompt bytes for a single planning unit. |`);
  lines.push(`| \`compile.planningUnitMaxObservedInputTokens\` | \`${DEFAULT_PLANNING_DECOMPOSITION_CONFIG.planningUnitMaxObservedInputTokens}\` | Maximum observed input-token budget per planning unit. |`);
  lines.push('| `compile.planningUnitMaxObservedTurns` | unset | Optional observed turn limit per planning unit. |');
  lines.push(`| \`compile.planningUnitMaxCompactHandoffBytes\` | \`${DEFAULT_PLANNING_DECOMPOSITION_CONFIG.planningUnitMaxCompactHandoffBytes}\` | Maximum compact handoff size carried between planning-unit steps. |`);
  lines.push(`| \`compile.planningUnitMaxLocalExplorationToolUses\` | \`${DEFAULT_PLANNING_DECOMPOSITION_CONFIG.planningUnitMaxLocalExplorationToolUses}\` | Maximum local exploration tool uses per planning unit. |`);
  lines.push(`| \`compile.planningUnitMaxCriteriaPerUnit\` | \`${DEFAULT_PLANNING_DECOMPOSITION_CONFIG.planningUnitMaxCriteriaPerUnit}\` | Maximum acceptance criteria assigned to one planning unit. |`);
  lines.push(`| \`compile.planningUnitMaxSubsystemsPerUnit\` | \`${DEFAULT_PLANNING_DECOMPOSITION_CONFIG.planningUnitMaxSubsystemsPerUnit}\` | Maximum subsystems assigned to one planning unit. |`);
  lines.push(`| \`compile.planningUnitMaxSplitAttemptsPerUnit\` | \`${DEFAULT_PLANNING_DECOMPOSITION_CONFIG.planningUnitMaxSplitAttemptsPerUnit}\` | Maximum split retries attempted for one planning unit. |`);
  lines.push('');
