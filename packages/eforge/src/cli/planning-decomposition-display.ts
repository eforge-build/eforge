...
export function renderPlanningDecompositionEventModel(event: DecompositionEvent): PlanningDecompositionEventModel {
  switch (event.type) {
    case 'planning:decomposition:start': return { headline: `Context-managed planning: ${event.unitCount} unit(s), ${event.edgeCount} edge(s), parallelism ${event.limits.parallelism}`, details: [`Graph: ${event.graphId}; root=${event.rootUnitId}`, `Limits: parallelism=${event.limits.parallelism}, maxDepth=${event.limits.maxDepth}, maxObservedInputTokens=${event.limits.maxObservedInputTokens}`] };
