...
The recovery dialog leads with sidecar verdict recovery and continue-and-repair. Daemon recovery auto-resume is default-off and separate from these confirmed manual controls; when enabled, only high-confidence compiled-artifact `continue-repair` may auto-queue within budget, and its evaluate/queued/stopped audit events remain visible in event history.
