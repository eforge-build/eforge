...
- **Compiled-build resume** - `fetchResumeEligibility` runs as a read-only preflight. When eligible, a confirmed **Resume compiled build** action calls `startResumeBuild`, queues the failed PRD for scheduler-owned resume, and shows queued resume status/metadata; when ineligible, the daemon `reason` is shown. After dispatch and a successful resume, the engine retires the failed queue item and reactivates skipped descendants automatically. If an activated resume fails, the engine rolls the PRD back to `failed/` and refreshes the recovery sidecar from the resumed-run evidence, or replaces/removes stale evidence when only degraded context is available.
