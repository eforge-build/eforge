...
- **Recommended recovery action** - the sidecar verdict drives a single confirmed primary action (`applySidecarRecovery`): `retry` → **Re-queue PRD**, `split` → **Enqueue successor PRD**, `abandon` → **Archive failed PRD**. A `manual` verdict shows **Manual review required** with no apply button.
- **Compiled-build resume** - `fetchResumeEligibility` runs as a read-only preflight. When eligible, a confirmed **Resume compiled build** action calls `startResumeBuild` and shows the returned session id and pid; when ineligible, the daemon `reason` is shown.
- **Advanced: queue-cascade retry/reactivation** - the lower-level repair lives in a collapsed advanced section. It states that it *moves the failed upstream back to the queue* and *may reactivate skipped descendants*, warns when the verdict is `manual` or confidence is `low`, fetches `fetchQueueRecoveryAnalysis` only once the section is opened, and applies via `applyQueueRecovery` (never sidecar apply) after an explicit confirmation.
