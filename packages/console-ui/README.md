...
**Completion transitions.** Once a queue-affecting mutation succeeds - or a sidecar reports durable applied state on open - `QueueRecoveryDialog` replaces the report/actions body with a stable completion panel (`src/components/recovery/recovery-completion-panel.tsx`). Completion state is a discriminated union (`sidecar-apply`, `continue-repair`, `already-applied`, `accepted-success`) carrying an optional `refreshError` string. Data fetching and mutation state stay in `QueueRecoveryDialog`; the completion panel and the accepted-success form (`src/components/recovery/accept-success-action.tsx`) are presentational. The panel includes a close button that calls `onOpenChange(false)` and cues the operator to inspect the Queue card after queue-affecting actions.
