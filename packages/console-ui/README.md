...
Lane-model behavior (planning vs validation vs gap-close lanes, PRD pill on the planning lane) is verified end-to-end by `multi-plan-gap-close.e2e.test.tsx` against `fixtures/multi-plan-gap-close.json`. Build detail timelines render compile-resilience diagnostics from the same event log: `planning:scope-context:failure` rows show compile scope/context failure details (including optional guard diagnostics or decomposition-exhaustion evidence), and `planning:decomposition:*` rows render context-managed planning unit summaries/details. Decomposition events and same-plan recovery lifecycle events (`plan:build:recovery:*`) remain display-only in the run-state reducer; synthesized `planning:complete`, build failure, and build complete events carry plan state. Run detail also derives a compile-level failure banner from those events when the compile stops before any `plan:build:failed` event exists.
...
daemon REST (Header auto-build and scheduler controls)
  ...
  → AutoBuildToggle            remains the desired-state control, labels paused scheduler state as auto-build on (scheduler paused), and displays projected recovery auto-resume decisions from AutoBuildState.recoveryAutoResume
...
**Completion transitions.** After sidecar apply, continue-and-repair queueing, or accepted-success apply, `QueueRecoveryDialog` replaces the report/actions body with a stable completion panel (`src/components/recovery/recovery-completion-panel.tsx`). Already-applied sidecar state on open is rendered as provenance in the report panel instead of forcing a completion transition. Queue-cascade apply results stay rendered in the advanced section after apply. Completion state is a discriminated union (`sidecar-apply`, `continue-repair`, `already-applied`, `accepted-success`) carrying an optional `refreshError` string. Data fetching and mutation state stay in `QueueRecoveryDialog`; the completion panel and the accepted-success form (`src/components/recovery/accept-success-action.tsx`) are presentational. The panel includes a close button that calls `onOpenChange(false)` and cues the operator to inspect the Queue card after queue-affecting actions.
