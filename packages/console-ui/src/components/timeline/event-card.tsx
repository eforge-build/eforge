...
import type { EforgeEvent } from '@/lib/run-state';
...
function classifyEvent(type: string, event: EforgeEvent): { cls: string; label: string } {
  ...
  // Verdict-bearing :complete events
...
function eventDetail(event: EforgeEvent): string | null {
  switch (event.type) {
    ...
    case 'extension:agent-task:failed':
    case 'extension:agent-task:cancelled': {
      const parts = [
        `Task: ${event.taskId}`,
        `Kind: ${event.taskKind}`,
      ];
      if ('message' in event && event.message) parts.push(`Message: ${event.message}`);
      if ('errorCode' in event && event.errorCode) parts.push(`Error code: ${event.errorCode}`);
      if ('durationMs' in event && event.durationMs !== undefined) parts.push(`Duration: ${event.durationMs}ms`);
...
function EventCardImpl({ event, startTime, showVerbose }: EventCardProps) {
  ...
  // Narrow recovery:complete for verdict chip
