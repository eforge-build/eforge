...
Use `registerConsoleWorkstation` for richer Console UI that needs browser interactivity. V1 source-authored workstations declare exactly one source: inline iframe `srcDoc` or a `frameBundle` with local bundle metadata (`root`, `entrypoint`, optional `styles`, optional `assets`, and optional `browserSdkVersion: 1`). Bundle roots must be `workstation-assets` or a child directory under `workstation-assets/`; `entrypoint`, `styles`, and `assets` paths are relative to that root. Bundle entries are projected to manifest `frameBundle.frameUrl` iframe navigations with the bridge token in the URL fragment. The daemon serves a generated bundle frame shell and declared files through immutable, content-addressed asset URLs; browsers never provide filesystem-relative asset paths. Workstations are not shared React components, private Console imports, or arbitrary asset bundles outside the workstation frame/asset contract. The workstation can call the parent-owned bridge at `window.eforge.invokeAction(actionId, input)` or use `invokeAction` from `@eforge-build/extension-sdk/browser` to invoke allowed extension actions.
