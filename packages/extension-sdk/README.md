...
| `registerConsoleWorkstation(workstation)` | Register a sandboxed Console workstation | Yes | Manifest/management metadata projection; Console renders iframe workstations under `/console/workstations` from `srcDoc` entries or daemon-projected `frameBundle.frameUrl` entries |
...
Use `registerConsoleWorkstation` for richer Console UI that needs browser interactivity. V1 source-authored workstations are iframe `srcDoc` documents rendered under `/console/workstations`; daemon-projected `frameBundle` entries render from the manifest `frameBundle.frameUrl` with the bridge token in the URL fragment. Workstations are not shared React components, private Console imports, or extension-authored arbitrary asset bundles. The workstation can call the parent-owned bridge at `window.eforge.invokeAction(actionId, input)` to invoke allowed extension actions.
