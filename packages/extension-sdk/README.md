...
| `registerConsoleWorkstation(workstation)` | Register a sandboxed Console workstation | Yes | Manifest/management metadata projection; Console renders iframe `srcDoc` workstations under `/console/workstations` |
...
The iframe HTML is trusted extension UI isolated by the Console iframe sandbox; it is not sanitized declarative content. Specify `allowedActions` to keep the bridge narrow. If the allowlist is omitted, the V1 manifest uses same-extension default behavior; Console rejects bridge calls outside the manifest allowlist. Separately served asset bundles, direct React component loading, private Console React imports, raw extension-owned HTTP routes, and extension-owned AI planning/chat APIs remain deferred. Reusable widgets should target a versioned workstation browser SDK or host-rendered slots when available, not private Console React imports.
