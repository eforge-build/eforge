...
`registerValidationProvider` providers run as fail-closed quality gates during the per-plan `validate` build stage. Normal validation failures — legacy non-empty string returns, structured `{ status: 'failed' }` results, and command-form non-zero exits — enter bounded in-plan recovery before terminal failure. Recovery uses the plan's review-fixer/evaluator path, spends the `review.maxRounds` budget, and reruns the provider suite from the first provider after each recovery attempt. Thrown errors/rejections, provider timeouts, and unexpected return shapes are hard failures that bypass recovery and emit `plan:build:failed` immediately. Unresolved recoverable failures still fail the plan. Prefer structured annotations with file/line data so recovery can target precise issues.
