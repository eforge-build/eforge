...
- **Session stream** - `subscribeWithSnapshot()` async-generator for consuming any daemon SSE stream with reconnect/backoff. Yields `{ kind: 'snapshot' }` on every connect (from `stream:hello`), `{ kind: 'event' }` for JSON events, and `{ kind: 'named' }` for other named SSE events. `DaemonStreamSnapshotSchema` is exported from `@eforge-build/client/events` for validating the hello snapshot shape, including queue `dispatchFailure`, queue hold/capability metadata, and failed-enqueue projections. `aggregateSessionSummary()` computes a `SessionSummary` from a flat event array. SSE callers use `API_ROUTES.daemonEvents` for daemon-wide streams or `buildPath(API_ROUTES.events, { runId })` for per-session streams.
- **Request/response types** - TypeScript interfaces and TypeBox schemas for daemon HTTP endpoints, including queue controls, failed-enqueue recovery, recovery guidance, scheduler pause/resume, extension contribution manifest availability metadata, action invocation (including unavailable failures), and daemon-owned agent-task contracts
