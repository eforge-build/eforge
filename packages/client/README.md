...
- Root CLI (`packages/eforge/src/cli/index.ts`, `packages/eforge/src/cli/daemon-lifecycle.ts`, `packages/eforge/src/cli/mcp-proxy.ts`, `packages/eforge/src/cli/mcp-extension-contributions.ts`)
...
- **Request/response and event contracts** - TypeScript interfaces and TypeBox schemas for daemon HTTP endpoints and streamed events, including queue controls, failed-enqueue recovery, `RecoveryGuidancePrepareResponse`, scheduler pause/resume, compile-resilience event contracts (`planning:preflight`, `planning:scope-context:failure`, `planning:decomposition:*`, `CompileArtifactSummary`, bounded diagnostics including optional guard diagnostics, decomposition failure evidence, and compile scope/context recovery options), extension contribution manifest availability metadata (including agent task contribution entries), action invocation (including unavailable failures), and daemon-owned agent-task contracts with legacy task-kind and contribution-reference start shapes
