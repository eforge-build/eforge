...
import { deriveReduceDigestTotalByteLimit, minimumReduceDigestPromptByteLength, PLANNING_MODULE_DOCS_WORK_PROMPT_RULE, PLANNING_MODULE_REVIEW_INTENT_PROMPT_RULE, PLANNING_MODULE_TEST_WORK_PROMPT_RULE, PLANNING_MODULE_WORK_DIGEST_MIRROR_RULE, validatePlanningReduceDigest } from './reduce-digest-contracts.js';
...
function formatSourceEvidence(records: PlanningSourceEvidenceRecord[]): string {
  ...
  return JSON.stringify(records.map((record) => ({
    ...
    accountedByteLength: record.accountedByteLength,
    localizationNeedIds: record.localizationNeedIds,
    localizationStatus: record.localizationStatus,
    localizationConfidence: record.localizationConfidence,
    candidateRank: record.candidateRank,
    ownershipRationale: record.ownershipRationale,
