...
`adaptive-rescope.ts` (stage-integration layer) wraps exploration in a bounded pre-map rescope loop. Exploration budgets scale with unresolved-need count (`planningUnitMaxLocalExplorationToolUses` stays the per-scope clamp, and the turn ceiling scales with the derived budget). A source with no literal path or directory needs (`0/0`) requires bounded repository inspection but is not, by itself, a low-confidence-share split signal. When deterministic signals already show a risky collapsed root atom from actual non-empty localization evidence or concrete subsystem boundaries beyond one planning unit, the loop may split before spending a broad repository-exploration pass. Generic inferred interfaces and surface labels trigger inspection, not decomposition. The cross-run ledger scales to cover scoped passes, reserves shallow budget for later scopes, and prioritizes scoped reruns by critical/unresolved need count rather than alphabetically. Submit-only grace prompts include prior read-only observations so budget-exhausted outcomes can preserve partial evidence.
...
`atom-map-runner.ts` runs tool-less atom planners over the graph, shared brief, and materialized evidence. Atom outputs carry fragments, module candidates, aspect updates, and reduce digests. Atom prompts receive a deterministic work profile and must propose the smallest coherent module set; generic lexical categories are not module boundaries. Typed module intent includes dependencies, documentation work, test work (`none`, `exercise-existing`, or `author-new`), exclusive test ownership (`builder`, `test-writer`, or `existing-only`), review depth, and rationale. Atom prompts should receive repository evidence only through compiler-provided briefs and materialized excerpts.
...
`reduce-runner.ts` and `reducer-agent.ts` run tool-less reducers over atom and child reduce outputs. Reducers synthesize coherent final fragments and module candidates, report conflicts, and report gaps. Every reduce node (including root reduction) has an explicit coalescing responsibility: it should merge compatible input proposals and retain splits only for concrete ownership, dependency, or compiler-recorded module-budget boundaries.

`module-boundary-budget.ts` is the shared source of context, criterion, and subsystem usage for reducer submissions and final proposal normalization. Reducer prompts receive the exact ceilings separately from their prompt-size budget. Node metadata omits criterion/aspect arrays already present in bounded child digests so large root reducers do not pay for duplicate traceability. The structured submission tool rejects an oversized module with deterministic diagnostics and permits a corrected submission within the same bounded agent run; it never splits modules itself or drops coverage. Child digest modules are rebuilt from the schema-validated module candidates before a parent consumes them, so a repaired boundary cannot leak stale pre-repair intent; the rebuilt digest is re-validated at submission time against the same byte and field budgets as the authored digest, so an accepted digest always fits the prompt slot the budget planner proved safe. Final normalization repeats the same checks and remains fail closed.
