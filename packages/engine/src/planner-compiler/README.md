...
A bounded repository exploration agent may add a structured `RepositoryExplorationOutcome` before compiler localization proceeds. Completed outcomes contribute normalized `SourceLocalizationInputHints`; `needs-rescope`, `budget-exhausted`, and `ambiguous` outcomes preserve unresolved need ids, reasons, attempted queries, candidate paths, rescope hints, notes, and tool-use counts for diagnostics. Echoed need, criterion, and aspect ids are checked against the deterministic source inventory/localization baseline, and unknown ids are dropped with machine-readable diagnostics instead of rejecting the whole outcome.

### Adaptive rescoping

`adaptive-rescope.ts` (stage-integration layer) wraps exploration in a bounded pre-map rescope loop. Exploration budgets scale with unresolved-need count (`planningUnitMaxLocalExplorationToolUses` stays the per-scope clamp, and the turn ceiling scales with the derived budget); a cross-run ledger caps total tool uses across attempts, charging at least one use per agent invocation so submit-only runs cannot loop unbounded. When a degraded outcome hits a risky collapsed root atom (low high-confidence share, subsystem diversity beyond one planning unit, or unresolved interface needs), the loop derives deterministic `PlanningRescopeDirective`s, splits the atom graph (`reason: 'rescope-split'`), reruns exploration only for unresolved scopes, and preserves scopes that already localize. The same directives are threaded into `runBoundedPlannerCompiler` so both layers derive the identical graph. Per-scope rerun outcomes are merged (most-degraded status wins) so a completed scope cannot mask a budget-exhausted sibling in diagnostics.

Terminal outcomes: exhausted attempts with critical needs (interface/entrypoint kinds or interface-key criteria) unresolved throw `AdaptiveRescopeFailClosedError` and fail the compile closed, persisting the rescope state to `rescope-fail-closed.json` in the plan-set directory (the main diagnostics artifact is never written on that path). Critical needs with no linked criteria cannot be targeted by scoped reruns and are excluded from the fail-closed gate with a warning. Budget exhaustion with only non-critical needs unresolved proceeds degraded (`exhausted-proceeded`); already-decomposed graphs and no-split-signal sources keep warning-only degradation. On successful compiles, rescope state lands in the `rescope` section of compiler diagnostics.
