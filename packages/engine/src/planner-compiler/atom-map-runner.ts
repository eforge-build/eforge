...
export interface RunPlanningAtomMapInput { graph: PlanningAtomGraph; inventory?: SourceInventory; sourceContent: string; cwd: string; harness: AgentHarness; agentOptions?: SdkPassthroughConfig & { maxTurns?: number }; aspects?: PlanningCriterionAspect[]; parallelism?: number; abortSignal?: AbortSignal }
export interface PlanningAtomMapResult { graphId: string; outputs: PlanningAtomOutput[]; coverage: PlanningAspectCoverageSummary; completedAtomIds: string[]; failedAtomIds: string[]; skippedAtomIds: string[]; blockedAtoms: BlockedPlanningAtom[]; readyAtomIds: string[]; mapComplete: boolean; validationErrors: string[]; events: EforgeEvent[]; iterations: number }
...
export async function runPlanningAtomMap(input: RunPlanningAtomMapInput): Promise<PlanningAtomMapResult> {
  ...
  while (true) {
    const decision = selectReadyPlanningAtoms({ graph: input.graph, completedAtomIds: completed, failedAtomIds: failed, skippedAtomIds: skipped, parallelism: input.parallelism });
    ...
    iterations += 1;
    const batchResults = await Promise.all(decision.readyAtomIds.map((atomId) => runAtom(input, requireTask(tasks, atomId))));
...
async function runAtom(input: RunPlanningAtomMapInput, task: PlanningAtomTask): Promise<AtomRunResult> {
  try {
    const result = await runPlanningAtomPlanner({ task, sourceContent: input.sourceContent, cwd: input.cwd, harness: input.harness, agentOptions: input.agentOptions, abortSignal: input.abortSignal });
...
function finish(input: RunPlanningAtomMapInput, state: { outputs: PlanningAtomOutput[]; events: EforgeEvent[]; completed: Set<string>; failed: Set<string>; skipped: Set<string>; validationErrors: string[]; iterations: number; blockedAtoms: BlockedPlanningAtom[]; readyAtomIds: string[] }): PlanningAtomMapResult {
  const summary = summarizePlanningAtomOutputs({ graph: input.graph, inventory: input.inventory, aspects: input.aspects, outputs: state.outputs });
