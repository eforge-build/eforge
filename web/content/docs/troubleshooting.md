...
4. Check whether the PRD has compiled artifacts and a feature branch. If it does, compiled-build resume (`eforge_resume_build` in Pi, `mcp__eforge__eforge_resume_build` in Claude Code, or `eforge resume <prdId>` via CLI) remains a separate option for rerunning the original compiled artifacts.
