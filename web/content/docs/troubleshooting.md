...
3. The verdict is one of:
   ...
   - `split` - enqueue a successor PRD from `suggestedSuccessorPrd` while keeping the failed PRD and sidecars as an audit trail; when the sidecar records landed or merged partial work, the successor starts from the preserved feature branch while targeting the original base branch
...
4. Check the sidecar's compiled-build resume fields. If `resumeEligibility.eligible` is true or `recoveryOptions` recommends `eforge_resume_build`, prefer that recommendation; if the sidecar says resume is ineligible, show the bounded reason. When older sidecars do not contain resume fields, use the read-only resume eligibility preflight (`GET /api/recover/resume-eligibility`) or Console preflight where available; in Claude Code or Pi, offer the resume command/tool only after explicit confirmation and let the daemon reject ineligible resumes rather than inferring eligibility manually from branch/artifact presence. Compiled-build resume (`eforge_resume_build` in Pi, `mcp__eforge__eforge_resume_build` in Claude Code, or `eforge resume <prdId>` via CLI) remains a separate queue-native option for rerunning the original compiled artifacts, not an `eforge_apply_recovery` verdict. Resume queues the failed PRD for scheduler dispatch, so it is subject to normal queue controls: parallelism limits, pause state, dependency gating, and profile routing. After dispatch and a successful resume, the engine automatically retires the failed queue item and reactivates skipped descendants using normal dependency semantics; after an activated failed resume, it rolls the PRD back to `failed/` and refreshes or degrades/removes the recovery sidecar so stale pre-resume evidence is not authoritative.
