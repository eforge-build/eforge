...
- Project-local backlog capture, search, board rendering, epics, dependencies, and promotion, backed by canonical private SQLite rows, with direct compact agent operations (`search-items`, `get-item`, `get-epic`, `capture-item`, and `update-item`) and projection flags for smaller payloads (epics, lane counts, sections, lifecycle rows, dependencies, dependents, and body text remain opt-in or omittable where supported).
- Recommendation refresh and backlog curation workflows backed by daemon-owned agent tasks, with server-derived recommendation actionability, lifecycle evidence, and duplicate planning guards.
...
- Generated recommendations plus read-time freshness/staleness and actionability projections from canonical lifecycle evidence.
