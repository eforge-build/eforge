...
The **Console dashboard** (`http://localhost:<port>/console/`) tracks cost, token usage, efficiency metrics, and pipeline progress in real time. Root UI requests redirect to Console, and the daemon keeps it available after the build completes so you can inspect results.
