...
7. **Re-check trust and boundaries** whenever code changes: changed project/team hashes must be re-trusted, workstation HTML is trusted iframe UI rather than sanitized declarative content, and raw routes, asset bundles, React injection, and extension-owned AI planning or chat APIs are not supported in the current release.
...
V1 intentionally keeps several boundaries out of scope: separately served frontend asset bundles, direct React component loading, private Console React imports, raw extension-owned HTTP routes, extension-owned AI planning/chat APIs, and arbitrary independently loaded frontend plugins are unsupported. Reusable widgets should use a documented workstation browser SDK or host-rendered slots if those surfaces are provided; do not import private Console React APIs.
