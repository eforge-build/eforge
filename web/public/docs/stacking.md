...
When enabled, the daemon triggers a sync from the project root after each build completes. If active builds overlap the stack candidates, the outcome is `deferred` and the daemon schedules a `retry-deferred` attempt after those builds complete.

> **Avoid `build.postMergeCommands: ["eforge stack sync"]` for automatic sync.** That path bypasses deferral and retry semantics. Use `stacking.sync.afterBuild: true` instead.

### Active-build deferral

When sync runs while active builds are in progress, branches whose worktrees overlap active builds are excluded and reported in `activeBuildSkips`. When all candidates are excluded, the outcome is `deferred`. Re-run `eforge stack sync` manually after active builds complete, or configure `stacking.sync.afterBuild: true` for automatic retry.
