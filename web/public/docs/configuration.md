...
**Validation timing**: the named profile is validated at execution time, not when the playbook is saved. `agent_profile` values on session plans are validated when the session plan is enqueued.
...
Individual builds and extension-originated enqueue requests can override the policy with `--landing-auto-merge` or `--no-landing-auto-merge` (CLI), or by sending `landingAutoMerge: true/false` in the enqueue body. Omitting the flag defers to the configured policy.
...
Override any bundled agent prompt by placing a `.md` file in `eforge/prompts/` with the same name as the bundled prompt file:
...
Prompt file names match the role name for most roles, but not all: the `formatter` role runs the intake prompt, so override it with `eforge/prompts/intake.md` (a `formatter.md` file has no effect).
