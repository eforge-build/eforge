...
- **Workspace layout**: The repo uses pnpm workspaces with packages in `packages/` (engine, eforge, monitor, monitor-ui, client, pi-eforge, scopes, input, docs-gen, extension-sdk), the public documentation site in `web/` (`@eforge-build/web`), and the Claude Code plugin in `eforge-plugin/`.
