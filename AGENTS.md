...
- **Daemon HTTP client, SSE subscriber, and response types live in `@eforge-build/client` (`packages/client/`).** Do not inline lockfile, daemon-request, or per-session SSE subscribe helpers in the CLI, MCP proxy, Pi extension, or anywhere else - import them from the shared package (`subscribeToSession` for single-session event streams). Bump `DAEMON_API_VERSION` in `packages/client/src/api-version.ts` when making breaking changes to the HTTP API surface.
