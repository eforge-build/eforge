...
Registers two validation providers using `registerValidationProvider`. Demonstrates both the function form (`type-check-gate`, using `ctx.exec.run` to invoke `pnpm type-check` programmatically) and the command form (`lint-gate`, using the `commands` array for simpler exit-code-is-failure subprocess dispatch). Documents fail-closed recovery semantics, the wall-clock timeout controlled by `extensions.validationProviderTimeoutMs`, the no-mutation contract, annotation precision guidance, and the runtime events emitted during provider execution.

> **Runtime note:** `registerValidationProvider` is runtime-supported. Providers execute during the per-plan `validate` build stage, after the implement stage and before the review stage, when `validate` is included in the build pipeline. Normal failures (non-empty string returns, structured `{ status: 'failed' }` results, and command-form non-zero exits) are recoverable using the plan's review-fixer/evaluator path and `review.maxRounds` budget; after each recovery attempt, the provider suite reruns from the first provider. Thrown errors/rejections, timeouts, and unexpected return shapes are hard failures that bypass recovery. Unresolved recoverable failures still emit `plan:build:failed`. See [`docs/extensions.md`](../../docs/extensions.md) — "Validation providers" and [`docs/extensions-api.md`](../../docs/extensions-api.md) — `registerValidationProvider`.
